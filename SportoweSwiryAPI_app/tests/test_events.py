...
def test_get_all_events_with_params(client, token, sample_event):
    response = client.get('/api/v1/all_events?fields=name,start,length_weeks,admin_id&sort=name&page=1&limit=1',
                            headers={
                                'Authorization': f'Bearer {token}'
                            })
    response_data = response.get_json()
    ...
    assert response_data['pagination'] == {
            'total_pages': 1,
            'total_records': 1,
            'current_page': '/api/v1/all_events?page=1&fields=name,start,length_weeks,admin_id&sort=name&limit=1',
            'current_page (number)': 1
        }
